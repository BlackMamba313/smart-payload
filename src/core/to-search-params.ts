...
const appendValue = (
  ...
  if (typeof normalized !== 'string') {
    throw new TypeError(`Binary value at key "${key}" cannot be serialized to URLSearchParams.`);
  }
...
export const toSearchParams = (input: unknown, options?: SerializeOptions): URLSearchParams => {
  const merged = mergeOptions(options);
  const params = new URLSearchParams();
  walk(input, params, merged);
  return params;
};
